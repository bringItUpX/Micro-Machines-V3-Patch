...
patchnum=bytes(b"\xc1\x83\xf8\x68")
orig=bytes(b"\xc1\x83\xf8\x28")
...
print("The script should patch all languages of the game executable,")
print("perhaps also of an other date of the file micro.exe")
print("------Hack:------")
print("The game has three occurrences of an assembler 'CMP 0x28, eax' command.")
print("This CMP command can handle numbers approximately till 0x7E")
...
print("------Usage:-----")
print("To work with this script you have to place the file micro.exe of any language of the game")
...
if os.path.exists("micro.exe"):
    ...
    print("If it is the german version of the file it should have 558080 bytes for micro.exe executable")
    offset = 0
    with open("micro.exe", "r+b") as filemicro:
        for offset in range(file_length_in_bytes):
            rightcontent = True
            filemicro.seek(offset + 0, begin)
            if (filemicro.read(4) != orig):
                rightcontent = False
            filemicro.seek(offset + 34, begin)
            if (filemicro.read(4) != orig):
                rightcontent = False
            filemicro.seek(offset + 82, begin)
            if (filemicro.read(4) != orig):
                rightcontent = False
        ...
        writepatch = True
        if (rightcontent == True):
            print("offset of the pattern found. It is at:")
            print(offset)
            print("it is the right micro.exe -> now patching the file.")
            filemicro.seek(offset + 0, begin)
            if filemicro.write(patchnum) == 4:
            ...
            print("I do not patch. The pattern can't be found in the micro.exe executable")
else:
